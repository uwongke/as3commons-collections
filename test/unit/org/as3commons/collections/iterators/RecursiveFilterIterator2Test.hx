...
/**
	 ...
	 */
class RecursiveFilterIterator2Test extends AbstractSpecialIteratorTestCase
{
    ...
    private function getRecursiveFilterIterator(flagFilter : String = "", flagChildren : String = "") : IRecursiveIterator
    {
        ...
        list, 
        (flagFilter != null) ? (flagFilter == "true") ? trueFilter : falseFilter : null, 
        (flagChildren != null) ? (flagChildren == "true") ? trueChildrenFilter : falseChildrenFilter : null);
    }
    
    private function trueFilter(item : Dynamic) : Bool
    {
        return cast((item), Node).active == true;
    }
    
    private function falseFilter(item : Dynamic) : Bool
    {
        return cast((item), Node).active == false;
    }
    
    private function trueChildrenFilter(item : Dynamic) : Bool
    {
        return cast((item), Node).active == true;
    }
    
    private function falseChildrenFilter(item : Dynamic) : Bool
    {
        return cast((item), Node).active == false;
    }
    ...
    public function test_filter() : Void
    {
        ...
        var expectedResult : Array<Dynamic> = [
        list0, 
        list0_0, 
        list0_0_0, 
        list0_0_1, 
...
class Node extends ArrayList
{
    ...
    @:allow(org.as3commons.collections.iterators)
    private function new(theName : String, theActive : Bool = true)
    {
        super();
        name = theName;
        active = theActive;
