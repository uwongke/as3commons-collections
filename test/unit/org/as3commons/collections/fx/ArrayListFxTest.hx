...
/**
	 ...
	 */
class ArrayListFxTest extends ArrayListTest
{
    ...
    public function test_replaceAt_fx() : Void
    {
        ...
        assertStrictlyEquals(TestItems.object5, event.item);
        assertEquals(1, event.numItems);
        assertTrue(Std.is(event.iterator(), IOrderedListIterator));
    ...
    public function test_iterator_addBefore_eventIndex() : Void
    {
        ...
        var iterator : IOrderedListIterator = try cast(_arrayList.iterator(), IOrderedListIterator) catch(e:Dynamic) null;
