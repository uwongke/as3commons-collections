...
/**
	 ...
	 */
class LinkedSetTest extends AbstractCollectionTestCase
{
    private var _orderedSet(get, never) : IOrderedSet;
    ...
    override public function fillCollection(items : Array<Dynamic>) : Void
    {
        ...
        for (item in items)
        {
            cast((collection), IOrderedSet).add(item);
        }
    }
    
    private function get__orderedSet() : IOrderedSet
    {
    ...
    {
        assertTrue(Std.is(_orderedSet, IOrderedSet));
    }
    ...
    // empty
    {
        ...
        assertTrue(null == _orderedSet.next(TestItems.object1Key));
        assertTrue(null == _orderedSet.previous(TestItems.object1Key));
        
        assertTrue(null == _orderedSet.next(TestItems.object2Key));
        assertTrue(null == _orderedSet.previous(TestItems.object2Key));
        
        assertTrue(null == _orderedSet.next(TestItems.object3Key));
        assertTrue(null == _orderedSet.previous(TestItems.object3Key));
