...
/**
	 ...
	 */
class SortedMapKeyIteratorTest extends AbstractIteratorTestCase
{
    ...
    {
        cast((collection), SortedMap).clear();
        for (item in items)
        {
            cast((collection), SortedMap).add(item, item);
        }
    }
    
    override public function getIterator(index : Int = 0) : IIterator
    {
        return cast((collection), SortedMap).keyIterator();
    }
    
    override public function toArray() : Array<Dynamic>
    {
        return cast((collection), SortedMap).keysToArray();
    }
