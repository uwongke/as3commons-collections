...
/**
	 ...
	 */
class LinkedSetIteratorTest extends AbstractIteratorTestCase
{
    ...
    {
        cast((collection), LinkedSet).clear();
        for (item in items)
        {
            cast((collection), LinkedSet).add(item);
        }
    }
    ...
    override public function getIterator(index : Int = 0) : IIterator
    {
        ...
        {
            var i : Int = 0;
            var node : LinkedNode = cast((collection), LinkedSet).firstNode_internal;
            while (node)
            {
                if (index == i)
                {
                    item = cast((node), LinkedNode).item;
                    break;
                }
                node = node.right;
                i++;
        ...
        var iterator : ICollectionIterator = try cast(cast((collection), LinkedSet).iterator(item), ICollectionIterator) catch(e:Dynamic) null;
    ...
    {
        return cast((collection), LinkedSet).toArray();
    }
    ...
    public function test_addBefore_withExistingItem() : Void
    {
        ...
        var iterator : IOrderedSetIterator = try cast(getIterator(), IOrderedSetIterator) catch(e:Dynamic) null;
        ...
        assertTrue(iterator.addBefore(TestItems.object5));
    }
    ...
    public function test_addAfter_withExistingItem() : Void
    {
        ...
        assertTrue(iterator.addAfter(TestItems.object5));
    }
